...
There are different legal reasoning/analysis methods, but professionals most commonly engage in both (1) **deductive reasoning** (i.e., syllogistic reasoning) and (2) [**inductive reasoning**](broken-reference) (in our context, this is reasoning by analogy).&#x20;

## Introduction

Most of the analysis you will perform in this class will involve deductive reasoning, which is also called syllogistic [^1]reasoning. What we will see is that deductive reasoning produces conclusions from a set of established premises (assertions): If A is true and B is true, then C must be true[^2].

The syllogism represents a fundamental depiction of the deductive reasoning process in law, which starts with a **general assertion or proposition and ends with a specific assertion (i.e., the conclusion)**. Imagine this approach as a funnel or an inverted pyramid[^3].&#x20;

<figure><img src="../../.gitbook/assets/inverted%20pyramid%203%20sections.png" alt="inverted pyramid divided into three parts. The major premise is at the base (i.e., at the top of the inverted pyramid), the minor premise is in the middle, and the conclusion is at the bottom." width="440"><figcaption><p>Inverted Pyramid</p></figcaption></figure>
...
> **Major Premise:** All men [^4]are mortal.
>
...
<figure><img src="../../.gitbook/assets/venn%20all%20men.png" alt="" width="515"><figcaption><p>Venn diagram illustrating the "Socrates syllogism"</p></figcaption></figure>
...
<figure><img src="../../.gitbook/assets/venn%20men%20bird.png" alt="" width="515"><figcaption><p>Venn diagram illustrating the invalidity of the "Joe's bird" syllogism</p></figcaption></figure>
...
Some logical flaws can be more subtle. Take the following syllogism, for example[^8]:
...
Because real-life circumstances can be complex, deductive reasoning _frequently_ requires applying more premises than exist with simple syllogisms. These circumstances require a **polysyllogism**—a syllogism with more than two premises—to include a more complex analysis to support a conclusion[^10].&#x20;

#### A "Socrates" Polysyllogism

Here is an example [^11]of a polysyllogism based on the Socrates syllogism, and the issue is whether Socrates is a god:
...
In their arguments, many people use **enthymemes**[^12]**,** which are implicit premises. Generally, arguers will use enthymemes for efficiency when they presume that the omitted premise is obvious[^13].&#x20;

In daily conversations, political arguments, legal reasoning (including judicial opinions[^14]), etc., enthymemes are common. For example[^15]:
...
Moving into the legal context in its plainest form, a legal syllogism in criminal cases looks like this[^20]:&#x20;
...
Here is a legal example[^22] of a polysyllogism:
...
>
> **Fact 3 (a "variable"):** Evidence [^24]from the investigation demonstrated that before killing Henry, Joseph broke into Henry's home, waited for Henry to return.
>
> **Fact 4 (a "variable"):** Evidence [^25]from the investigation demonstrated Joseph did not act in self-defense at the time he killed Henry.
>
> **Analysis:**&#x20;
>
...
Common to judicial opinions, some arguments require a little extra work to find the syllogism.\*[^26] For example, based on the question of whether there is a constitutional right to an abortion, in _Roe v. Wade_[^27], the majority wrote:
...
Now that we have taken a look at deductive or rule-based reasoning, we will move on to a type of inductive reasoning: [**analogical reasoning**](broken-reference).
...
[Introduction to Deductive (Rule-Based) Reasoning](broken-reference) © 2024 by Matthew L. Mac Kelly is licensed under [CC BY-NC-SA 4.0](https://creativecommons.org/licenses/by-nc-sa/4.0/?ref=chooser-v1), except where otherwise noted.





[^1]: See [Syllogisms](1.1.1-introduction-to-deductive-rule-based-reasoning.md#syllogisms); [Kulicki (2020)](broken-reference).

[^2]: [Aldisert et al.](broken-reference), 3.

[^3]: [Swisher](broken-reference), 537, 539 n.18.

[^4]: i.e., humans

[^5]: i.e., human

[^6]: [Fields ](broken-reference)(2024).

[^7]: [Fields ](broken-reference)(2024).
...
[^12]: [Aldisert et al.](broken-reference), 8; [Nordquist (2018)](broken-reference).&#x20;

[^13]: [Aldisert et al.](broken-reference), 8.

[^14]: [Aldisert et al.](broken-reference), 8.
...
[^19]: See generallly, [Ramee](broken-reference).

[^20]: See [Aldisert et al.](broken-reference), 6.

[^21]: This example is adapted from [Swisher](broken-reference), 536.&#x20;

[^22]: [Swisher](broken-reference), 536.
...
[^26]: [Aldisert et al.](broken-reference), 7.

[^27]: The fact that [_Roe v. Wade_](https://scholar.google.com/scholar_case?case=12334123945835207673\&q=roe+v.+wade\&hl=en\&as_sdt=6,50) was overturned by [_Dobbs v. Jackson Women's Health Organization_](https://scholar.google.com/scholar_case?case=10996775398954026979\&hl=en\&as_sdt=6,50) in 2022 does not affect the purpose and legitimacy of this example.
